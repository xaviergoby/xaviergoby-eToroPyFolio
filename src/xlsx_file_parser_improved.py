...
class AccStatementParser:
	...
	def _parse_order_fin_instrument_type(self, closed_pos_id):
		"""
		:param closed_order_action_desc: e.g. "Buy Advanced Micro Devices Inc" or "Buy Bitcoin"
		:return:
		"""
		order_fin_instrument_name = self._closed_pos_sheet_df["Action"][self._closed_pos_sheet_df["Position ID"] == 674813914].values[0].lstrip("Buy").lstrip()
		# crypto_symbols_list = utils.read_json_file("cryptocurrencies.json")
		if order_fin_instrument_name in list(self._all_crypto_symbols_list.values()):
			return "crypto"
		else:
			return "stock"
